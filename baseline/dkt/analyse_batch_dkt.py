...
if __name__ == '__main__':
    ...
    paired_and_single_counter = 0
    single_counter = 0
    paired_counter = 0
    counter = 0
    for b in dl:
        kc_seq = b['ktbench_kc_seq'].squeeze(0)
        kc_seq_mask = b['ktbench_kc_seq_mask'].squeeze(0)
        res =  trained_model.ktbench_trace(**b).squeeze(0)
        print(res.shape)
        ...
        for kcs, kcs_mask in zip(kc_seq, kc_seq_mask):
            for kc, mask in zip(kcs, kcs_mask):
                ...
                if mask == 0:
                    break
                kt = res[idx]
                diff = torch.abs(kt-prev_kt)
                boolidx = torch.nonzero(diff> 0.5).flatten()
                prev_kt = kt

                if (prev_kc, kc) in paired_dict:
                    print(diff[kc])
                prev_kc = kc
                for x in boolidx:
                    continue
    ...
    print(paired_and_single_counter/counter)
    print(paired_counter/counter)
    print(single_counter/counter)