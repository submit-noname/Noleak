...
from ..utils.teach_lstm import LSTMModel
...
class DKT_AD(BaseModel):
    ...
    def build_model(self):
        ...
        def teacher_fn(seq, prev_hidden_context, current_x, **kwargs):
            exer_seq = kwargs['exer_seq'][:, seq, None]
            hidden, _ = prev_hidden_context
            hidden = hidden.unsqueeze(1)
            output = self.dropout_layer(hidden)
            y_pd = self.fc_layer(output).sigmoid()
            y_pd = y_pd.gather(
                index=exer_seq.unsqueeze(-1), dim=2
            ).squeeze(dim=-1)
            y_pd = torch.round(y_pd)
