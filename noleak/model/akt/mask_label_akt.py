...
class AKT_ML(BaseModel):
    def __init__(self, cfg, params=Params()):
        ...
        if self.n_pid > 0:
            ...
            self.qa_embed_diff = nn.Embedding(3 * self.n_question + 1, embed_l)
        # n_question+1 ,d_model
        self.q_embed = nn.Embedding(self.n_question+1, embed_l)
        if self.separate_qa:
            self.qa_embed = nn.Embedding(3*self.n_question+1, embed_l)
        else:
    ...
    def forward(self, **kwargs):
        ...
        labels = kwargs['ktbench_masked_label_unfold_seq']
